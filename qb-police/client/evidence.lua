...
CreateThread(function()
    while true do
        Wait(10)
        if LocalPlayer.state.isLoggedIn then
            if (PlayerJob.name == 'police' or PlayerJob.name == 'sheriff' or PlayerJob.name == 'fbi' or PlayerJob.name == 'swat') then
...
RegisterNetEvent('police:client:createbox', function(slot)
    local input = lib.inputDialog('Create Evidence Box', {
        {type = 'input', label = 'CitizenID', description = 'add citizenID'},
    })
    if input then 
        TriggerServerEvent('police:server:createbox', {
            citizenid = input[1],
            slot = slot
        })
    end
...
RegisterNetEvent('police:client:boxinteractions', function(item, grade)
    if not item or not grade then return end 
    lib.registerContext({
        id = 'Evidence_Box',
        title = 'Evidence Box',
        options = {
          {
            title = 'Open',
            description = 'Open Evidence Box',
            icon = 'box',
            onSelect = function()
                if item.info and item.info.caseid then 
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", 'Evidence_Box '..item.info.caseid, {
                        maxweight = 4000000,
                        slots = 40,
                    })
                    TriggerEvent("inventory:client:SetCurrentStash", 'Evidence_Box '..item.info.caseid)
                end
            end,
          },
          {
            title = 'Destroy',
            description = 'Destroy Evidence Box',
            icon = 'times',
            onSelect = function()
                local allowed = {
                    [10] = true,
                ...
                if allowed[tonumber(grade)] then 
                    local alert = lib.alertDialog({
                        header = 'Destroy Evidence Box',
                        content = 'Are you sure you want to destroy evidence box #  \n[ '..item.info.caseid..' ]',
                        centered = true,
                        cancel = true
                    })
                    if alert == 'confirm' then 
                        TriggerServerEvent('police:server:deleteBox', item)
                    end
                else
